...
function CustomHead({ ...props }: HeadProps) {
    ...
    return (
      // Usamos um fragmento para não adicionar um `thead` extra. O `table` do DayPicker lida com isso.
      // O `head_row` no `classNames` vai aplicar o flex.
      <>
        {/* Renderiza a linha de dias da semana */}
        <div className={cn(buttonVariants({ variant: "outline" }), "flex w-full mt-4")}>
            {weekDays.map((day, i) => (
                <div
                    key={i}
                    scope="col"
