...
export default function RescisaoSimulacaoModal({ isOpen, onClose, funcionario, tempoDeCasaEmMeses }: RescisaoSimulacaoModalProps) {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setMotivo('demissao_sem_justa_causa');
    }
  }, [isOpen]);
  ...
  useEffect(() => {
    ...
    decimoTerceiroProporcional = (salarioBase / 12) * mesesTrabalhadosNoAno;
    ...
    feriasProporcionais = (salarioBase / 12) * mesesPeriodoAtual;
    ...
    if (motivo === 'demissao_sem_justa_causa') {
      ...
      // O empregado cumpre ou indeniza o aviso prévio, para a simulação, consideramos que ele indeniza
      avisoPrevio = -salarioBase; 
      if (periodosAquisitivosCompletos >= 1) {
    ...
    let total = saldoSalario + decimoTerceiroProporcional + totalFerias + tercoFerias;
    if (motivo === 'demissao_sem_justa_causa') {
        total += avisoPrevio;
    }
  ...
  }, [dataRescisao, motivo, funcionario, tempoDeCasaEmMeses]);
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Calcule uma estimativa das verbas rescisórias para {funcionario.nome}. Os valores são aproximados.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="py-4 space-y-4">
            <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                    <Label htmlFor="motivo-rescisao">Motivo</Label>
                     <Select value={motivo} onValueChange={setMotivo}>
                        <SelectTrigger id="motivo-rescisao">
                            <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
            ...
            <div className="space-y-2 rounded-lg bg-muted/50 p-4">
                ...
                <div className="space-y-1 text-sm">
                    ...
                    {motivo === 'demissao_sem_justa_causa' && <div className="flex justify-between"><span>Aviso Prévio Indenizado:</span> <span className="font-mono">{formatCurrency(verbas.avisoPrevio)}</span></div>}
                </div>
